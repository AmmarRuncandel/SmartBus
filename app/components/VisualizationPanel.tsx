...
interface VisualizationPanelProps {
  ...
  hybrid: AlgorithmResult | null;
  start: NodeId;
  destination: NodeId;
  mode: 'comparison' | 'hybrid';
}
...
export default function VisualizationPanel({
  ...
  const hasComparisonResults = astar && ucs;
  const hasHybridResult = !!hybrid;
  const shouldShowEmpty = mode === 'comparison' ? !hasComparisonResults : !hasHybridResult;
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: 'rgba(26,26,26,0.5)' }}>
            {mode === 'comparison'
              ...
              : 'Satu hasil pencarian dari Hybrid A*-UCS'}
          </p>
        </div>
      </div>
        ...
        <>
          {mode === 'comparison' ? (
            ...
            <>
              {hybrid ? (
                <>
                  <TampilJalur result={hybrid} label="Jalur Hybrid A*-UCS" color="#7A1FA2" />
                  <LogEkspansi
                    title="Urutan Kunjungan Hybrid"
                    log={hybrid.expansionLog}
                    color="#7A1FA2"
                    tampilkanFCost
                  />
                </>
